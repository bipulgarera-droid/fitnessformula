...
const BookingModal: React.FC<BookingModalProps> = ({ isOpen, onClose, onConfirm, initialData }) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    setFormData({
      name: '', phone: '', email: '', service: '1 Week Trial', date: '', time: '', notes: ''
    });
  };
  ...
  return (
    <div className="fixed inset-0 z-[60] flex items-center justify-center px-4 overflow-y-auto">
      ...
      <div className="bg-white rounded-2xl p-6 md:p-8 w-full max-w-lg relative shadow-2xl animate-fade-in-up my-8">
        ...
        <h3 className="text-3xl font-serif italic mb-2">Book Free Consultation</h3>
        <p className="text-gray-500 text-sm mb-6">Confirm your details below. We'll contact you to discuss your personalized plan.</p>
        ...
        <form className="space-y-4" onSubmit={handleSubmit}>
          ...
          {/* Service Selection */}
          <div>
            <label className="block text-xs uppercase tracking-wider text-gray-500 mb-1">Select Program / Course</label>
            <select
              name="service"
              ...
              className="w-full border-b border-gray-300 py-2 focus:outline-none focus:border-black transition-colors bg-transparent text-sm"
            >
              <option>1 Week Trial</option>
              <option>21 Days Challenge</option>
              <option>60 Days Transformation</option>
              <option>12 KGS Transformation</option>
              <option>16 KGS Transformation</option>
              <option>20 KGS Transformation</option>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="block text-xs uppercase tracking-wider text-gray-500 mb-1">Preferred Callback Date</label>
              <input
                name="date"
                value={formData.date}
                onChange={handleChange}
